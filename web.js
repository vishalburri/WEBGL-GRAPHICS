...
makeModel('aqua', 0, 0, -900, 1800.0, 900.5, 2400.0, 0, 0, 0, 'aqua.data', -1);
//makeModel('bed', 1, -3.8 , 10, 400.0, 0.1, 4.0, 0, 0, 0, 'aqua1.data', 0);
makeModel(fisharr[0], 0, 0, 10, 1, 1, 1, 0, 0, 0.5, 'fish.data', 0);
makeModel(fisharr[1], 39, -2, 30, 0.5, 0.5, 0.5, 0, 0, 0.5, 'fish.data', 1);

makeModel(fisharr[2], 34, 3, 70, 70, 70, 70, 0, 0, 0.5, 'gfish.data', 2);
makeModel(fisharr[3], 18, 13, 50, 70, 70, 70, 0, 0, 0.5, 'gfish.data', 3);

makeModel(fisharr[4], -27, 14, 30, 70, 70, 70, 0, 0, 0.5, 'gfish.data', 4);
makeModel(fisharr[5], -14, -2, 20, 70, 70, 70, 0, 0, 0.5, 'gfish.data', 5);


makeModel('rock1', 10, -10, 20, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);
makeModel('rock2', 13, -10, 20, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);
makeModel('rock0', 16, -10, 20, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);
makeModel('rock7', 19, -10, 20, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);

makeModel('rock3', -15, -10, 40, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);
makeModel('rock4', -17, -10, 40, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);
makeModel('rock5', 10, -10, 60, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);
makeModel('rock6', 12, -10, 60, 1, 1, 1, 0, 0, 0, 'RockSet.data', -1);

for(var i=0;i<100;i++){
  makeModel(i, 1, -14, -200+i*10, 50, 5, 5, 0, 0, 0, 'Grass_01.data', -1);

}
for(var i=100;i<200;i++){
  makeModel(i, 60, -14, -200+(i-100)*10, 50, 5, 5, 0, 0, 0, 'Grass_01.data', -1);

}
for(var i=200;i<300;i++){
  makeModel(i, -50, -14, -200+(i-200)*10, 50, 5, 5, 0, 0, 0, 'Grass_01.data', -1);

}
for(var i=300;i<400;i++){
  makeModel(i, -100, -14, -200+(i-300)*10, 50, 5, 5, 0, 0, 0, 'Grass_01.data', -1);

}
for(var i=400;i<500;i++){
  makeModel(i, -150, -14, -200+(i-400)*10, 50, 5, 5, 0, 0, 0, 'Grass_01.data', -1);

}
for(var i=500;i<600;i++){
  makeModel(i, 110, -14, -200+(i-500)*10, 50, 5, 5, 0, 0, 0, 'Grass_01.data', -1);

}
for(var i=600;i<700;i++){
  makeModel(i, 165, -14, -200+(i-600)*10, 50, 5, 5, 0, 0, 0, 'Grass_01.data', -1);

}

makeModel(fisharr[6], 24, 13, 20, 0.2, 0.2, 0.2, 0, 0, 0.5, 'ORCA.data', 6);
makeModel(fisharr[7], 15, -2, 40, 0.2, 0.2, 0.2, 0, 0, 0.5, 'ORCA.data', 7);
makeModel(fisharr[8], -4, 14, 60, 0.2, 0.2, 0.2, 0, 0, 0.5, 'ORCA.data', 8);
makeModel(fisharr[9], -5, -3,70, 0.2, 0.2, 0.2, 0, 0, 0.5, 'ORCA.data', 9);

document.onkeydown = handleKeyDown;
document.onmouse = handlemouse;
 setInterval(drawScene, 1); //(1000/15) fps
 //drawScene();
...
function createModel(name, x_pos, y_pos, z_pos, x_scale, y_scale, z_scale, speed_x, speed_y, speed_z, filedata, filename, iscoin) //Create object from blender
{
 ...
 // console.log(models[name].speed[0]);
  if(iscoin>=0){
     ...
     angle=0;
     if(camtype==2 && name==fisharr[index])
...
for(var key in models){
  ...
  if(model['name']=='egg'){
          models['egg'].center[1]-=models['egg'].center[1]*models['egg'].speed[1];
  }
...
function handleKeyDown(event) {
        ...
        if (event.keyCode == 38 && camtype==2) {
          ...
          for(var key in models){
            ...
            if(model['name']==pfish){
              ...
              zposz+=0.1;
              if(revdir[index]==0){
              model['center'][0]+=0.3*Math.sin(ang*Math.PI/180);
              model['center'][2]+=0.3*Math.cos(ang*Math.PI/180);
        ...
        if(event.keyCode == 65) {
          ...
          ang+=1;
          if(camtype==2){
            models[fisharr[index]].center[0]+=8.5*Math.PI/180*Math.cos(Math.PI/180)*(revdir[index]-0.5)*-2;
            models[fisharr[index]].center[2]-=8.5*Math.PI/180*Math.sin(Math.PI/180)*(revdir[index]-0.5)*-2;
        ...
        if(event.keyCode==71 && camtype==2){
        makeModel('egg',models[fisharr[index]].center[0],models[fisharr[index]].center[1],models[fisharr[index]].center[2],0.1,0.1,0.1,0,0.05,0,'sphere.data',-1);
        }
         if(event.keyCode == 83){
 ...
 */
 ...
 var projectionMatrix = m4.perspective(fieldOfViewRadians, aspect, zNear, zFar);
    ...
    if (camtype==2){
      var temp;
      var d=Math.sqrt(1.44+64);
      var b=ang-180*Math.atan(1.2/8)/Math.PI;
      if(revdir[index]==0)
        temp=1;
      if(revdir[index]==1)
        temp=-1;
      var x=models[fisharr[index]].center[0]-d*temp*Math.sin(b*Math.PI/180);
      var z=models[fisharr[index]].center[2]-d*temp*Math.cos(b*Math.PI/180);
